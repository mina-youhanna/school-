...
<style>
    ...
    .news-ticker-container {
        ...
        position: relative;
        margin-top: 0;
        /* Changed from 80px to 0 for correct positioning below fixed navbar */
        z-index: 1000;
    ...
    .event-content {
        ...
        /* قلل padding-top من 22px إلى 10px مثلاً */
        text-align: center;
        background: none;
        flex: 1 1 auto;
    }
    ...
    .event-cta {
        ...
        justify-content: center;
        margin-top: 0;
        /* أو margin-top: auto إذا أردت الزر في الأسفل دائماً */
    }
    ...
    .admin-news-btn:hover i,
    .admin-verse-btn:hover i {
        color: #ffd700;
    }
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.newsItems = [{
                id: 1,
                text: "مرحباً بكم في موقعنا الرسمي الجديد"
            },
            {
                id: 2,
                text: "قداس الأحد القادم الساعة 9 صباحاً"
            },
            {
                id: 3,
                text: "نهضة صلاة للسيدة العذراء الأسبوع القادم"
            },
            {
                id: 4,
                text: "عظة الأب أنطونيوس الجمعة القادمة"
            },
            {
                id: 5,
                text: "حملة التبرع بالملابس مستمرة حتى نهاية الشهر"
            }
        ];
    ...
    window.verses = [{
            id: 1,
            text: "هَذَا هُوَ الْيَوْمُ الَّذِي صَنَعَهُ الرَّبُّ. نَبْتَهِجُ وَنَفْرَحُ فِيهِ",
            reference: "مزمور 118: 24"
        },
        {
            id: 2,
        ...
        {
            id: 3,
            text: "كُلُّ شَيْءٍ يُمْكِنُ لِلَّذِي يُؤْمِنُ",
            reference: "مر 9: 23"
        }
    ];
    ...
    window.editService = function(idx) {
        alert('تعديل الخدمة (سيتم لاحقًا)');
    };
    ...
    window.updateServiceTitle = function(i, val) {
        window.services[i].title = val;
        window.renderServices();
    };
    window.updateServiceDesc = function(i, val) {
        window.services[i].desc = val;
        window.renderServices();
    };
    window.updateServiceIcon = function(i, val) {
        window.services[i].icon = val;
        window.renderServices();
    };
    window.deleteService = function(idx, fromManage) {
        if (confirm('حذف الخدمة؟')) {
            window.services.splice(idx, 1);
            window.renderServices();
    ...
    window.updateEventTitle = function(i, val) {
        window.events[i].title = val;
        window.renderEvents();
    };
    window.updateEventDetails = function(i, val) {
        window.events[i].details = val;
        window.renderEvents();
    };
    window.updateEventDate = function(i, val) {
        window.events[i].date = val;
        window.renderEvents();
    };
    window.deleteEvent = function(idx, fromManage) {
        if (confirm('حذف الفعالية؟')) {
            window.events.splice(idx, 1);
            window.renderEvents();
