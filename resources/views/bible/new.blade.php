...
<div class="container">
    <div class="bible-main-title">العهد الجديد</div>

    <!-- قائمة منسدلة لاختيار السفر -->
    <div class="bible-selects-row">
        <!-- اختيار السفر -->
        <form method="get" action="{{ route('bible.new.chapters', 0) }}" id="bookForm" style="margin:0;">
            <select name="book" id="bookSelect" class="form-select">
                <option value="">اختر السفر</option>
                @foreach($books as $b)
                    <option value="{{ $b->book_num }}" @if(isset($book_number) && $book_number == $b->book_num) selected @endif>
                        {{ $b->book_name }}
                    </option>
                @endforeach
            </select>
...
<script>
    document.getElementById('bookSelect').addEventListener('change', function() {
        ...
        if(bookNumber) {
            window.location.href = '/new-testament/' + bookNumber;
        } else {
            window.location.href = '/new-testament';
        }
    });
    ...
    if(chapterSelect) {
        chapterSelect.addEventListener('change', function() {
            ...
            if(chapterNumber && bookNumber) {
                window.location.href = '/new-testament/' + bookNumber + '/' + chapterNumber;
            }
        });
    }
