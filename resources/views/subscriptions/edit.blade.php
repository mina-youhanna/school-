...
<div class="subscription-form-container" dir="rtl">
    ...
    <h2 class="form-header">تعديل الاشتراك</h2>
    ...
    <div style="background: linear-gradient(135deg, rgba(255, 215, 0, 0.1), rgba(255, 215, 0, 0.05)); border: 1px solid rgba(255, 215, 0, 0.3); border-radius: 10px; padding: 20px; margin-bottom: 30px; text-align: center;">
        ...
        <div style="color: white; font-size: 1.1em; line-height: 1.6;">
            ...
            • يمكن تعديل القيمة والتواريخ حسب الحاجة
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.subscriptions.update', $subscription) }}">
        ...
        <div class="form-group">
            <label for="amount">قيمة الاشتراك (جنيه):</label>
            <input type="number" name="amount" id="amount" value="{{ old('amount', $subscription->amount) }}" min="0" step="0.01" required>
            @error('amount')
                <div class="error-message">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="date" name="subscription_date" id="subscription_date" value="{{ old('subscription_date', $subscription->subscription_date->format('Y-m-d')) }}" required>
        ...
        <div class="form-group">
            ...
            <input type="date" name="expiry_date" id="expiry_date" value="{{ old('expiry_date', $subscription->expiry_date->format('Y-m-d')) }}" required>
            @error('expiry_date')
                <div class="error-message">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <select name="status" id="status" required>
                ...
                <option value="pending" {{ old('status', $subscription->status) == 'pending' ? 'selected' : '' }}>في الانتظار</option>
            </select>
            @error('status')
                <div class="error-message">{{ $message }}</div>
            @enderror
        ...
        <div class="form-group">
            ...
            <textarea name="notes" id="notes" placeholder="أي ملاحظات إضافية...">{{ old('notes', $subscription->notes) }}</textarea>
            @error('notes')
                <div class="error-message">{{ $message }}</div>
            @enderror
        </div>
